...
real-world energy use (Table~\ref{tab:rho_inputs}),
\[
...
\fignotes{European, tax-inclusive, same semester.}
\end{table}

Three choices need stating. \emph{(i) Tax-inclusive prices.} The baseline has
...
taxation of the two fuels, which the ETS acts on.
...
sides are on the same basis. \emph{(iii) Fleet averages.} A matched compact pair
(electric $17$~kWh/100km, gasoline $6.2$~L/100km) gives $0.49$, so the ratio does
not depend on vehicle-size composition. As an internal check, $0.21$~kWh/km is
...
$\psi_g$ re-solves as $\varrho$ varies (Table~\ref{tab:rho_sensitivity}). Across
the whole plausible range the policy CEV ranking is unchanged (flat transfer $\succ$ Slutsky $\succ$ ex-post cap
...
\begin{table}[H]\centering
\figtitle{Sensitivity of the baseline to $\varrho$}
\small
...
peak $\Delta D^G$ (pp)          & 7.74  & 8.03  & 8.39  & 8.74  & 9.14 \\
peak output loss $-y$ (\%)      & 2.86  & 2.87  & 2.88  & 2.89  & 2.90 \\
\bottomrule
\end{tabular}
\label{tab:rho_sensitivity}
\fignotes{Price shock, import booking. $\bar D^G=5\%$ imposed throughout, $\psi_g$
re-solved as $\varrho$ varies.}
\end{table}
