...
labour productivity $e$, and the durable technology $g\in\{B,G\}$ it operates.
...
denote $g_{-1}$. Three events then occur, in this order, before it consumes.

\begin{enumerate}
\item \emph{Breakdown.} A green durable fails with probability $\delta_g$ and reverts
to brown; a brown durable is unaffected. We write $g_-$ for the technology the
household holds \emph{after} this draw---the technology it brings to the decision.
Hence $g_-=B$ whenever $g_{-1}=B$, while $g_-=G$ with probability $1-\delta_g$ when
$g_{-1}=G$.
...
technology $g$ it will operate together with its consumption and saving. A household
holding brown ($g_-=B$) may keep brown or pay the switching cost $\psi_g$ to adopt
green; a household holding green ($g_-=G$) operates it, since running an owned green
...
forward---$g$ becomes next period's $g_{-1}$---so the within-period sequence is
%
\[
\underbrace{g_{-1}}_{\text{held on entry}}
\;\xrightarrow{\ \text{breakdown}\ }\;
\underbrace{g_-}_{\text{held at the decision}}
\;\xrightarrow{\ \text{choice}\ }\;
\underbrace{g}_{\text{operated, carried forward}}.
\]
%
Two binary objects thus describe the technology: the holding at the decision, $g_-$,
and the operated technology, $g$. The switching cost is paid exactly when a brown
holder adopts, that is when $g_-=B$ and $g=G$; we write $\I[g_-=B,g=G]$ for a switcher
and $\I[g=G]$ for a household operating green. The feasible choices are
%
\[
\mathcal F(g_-)=
\begin{cases}
\{B,\,G\} & g_-=B \quad(\text{keep brown, or pay $\psi_g$ to adopt}),\\[2pt]
\{G\} & g_-=G \quad(\text{operate the green durable}).
\end{cases}
\]
...
The departure from ARS is that energy comes at a technology-specific price: a
...
%
\begin{equation}
p^E_g=\PEB+\I[g=G]\,\bigl(\PEG-\PEB\bigr),
\label{eq:pEd}
\end{equation}
\begin{equation}
p_g=\Bigl[1+\alpha_E\bigl((p^E_g)^{1-\eta_E}-(\PEB)^{1-\eta_E}\bigr)\Bigr]^{\frac{1}{1-\eta_E}},
\qquad
p_g=\bigl(p^E_g/\PEB\bigr)^{\alpha_E}\ \text{ for }\eta_E=1.
\label{eq:pd}
\end{equation}
%
...
At the consumption stage the household has settled its technology: it operates $g$,
having reached the decision holding $g_-$. Its cash on hand is
%
\begin{equation}
\coh(g_-,g,e,a)=(1+r)\,a+e\,Z+T-\psi_g\,\I[g_-=B,\,g=G],
\label{eq:coh}
\end{equation}
%
the sum of gross asset returns at the ex-post real return $r$, after-tax labour
income $e\,Z$---where $Z\equiv(1-\tau^L)wn$ is aggregate after-tax labour income
distributed by productivity, as in \citet{ARS2024}---transfers, and, for a switcher
only, the adoption cost.\footnote{Formally the household solves in units of the domestic good,
...
%
\begin{equation}
\max_{g\in\mathcal F(g_-)}\;\;\max_{c,\;a'\ge0}\;
\Bigl\{\,u(c)+\beta_i\,\E\bigl[\bar V(g_-',e',a')\bigr]\Bigr\}
\quad\text{s.t.}\quad p_g\,c+a'=\coh(g_-,g,e,a),
\label{eq:joint}
\end{equation}
%
with $\bar V$ the continuation value (defined in \eqref{eq:logit}) and next period's
holding $g_-'$ given by breakdown applied to the operated technology $g$. The nested
maxima are not a temporal sequence---nothing is realised between them---but the
structure of a discrete--continuous choice: for each technology we solve the
continuous problem, obtaining the technology-conditional value $V(g\mid g_-,e,a)$
(the inner maximum, in which the origin $g_-$ enters only through the switching cost),
and the household then selects the technology (Section~\ref{sec:adoption}). Only three
continuous problems are ever relevant: a brown household ($p_B\equiv1$, no switching
cost), a green incumbent ($p_G$, no switching cost), and a green adopter ($p_G$, cash
on hand reduced by $\psi_g$). Each satisfies the usual Euler and envelope
conditions,
%
\begin{equation}
u'(c)=\beta_i\,p_g\,\E\bigl[\bar V_a(g_-',e',a')\bigr],
\qquad
V_a=(1+r)\,u'(c)/p_g,
\label{eq:euler}
\end{equation}
...
%
\begin{equation}
c_E(g)=\alpha_E\bigl(p^E_g/p_g\bigr)^{-\eta_E}c,
\qquad
c_{HF}(g)=(1-\alpha_E)\bigl(p_{HF/P}/p_g\bigr)^{-\eta_E}c,
\label{eq:hh-demands}
\end{equation}
%
and brown versus green energy use follow the technology,
$c^B_E=c_E\,(1-\I[g=G])$ and $c^G_E=c_E\,\I[g=G]$.
...
problem. A household holding $g_-$ compares the technology-conditional values
$V(g\mid g_-,\cdot)$; with an i.i.d.\ extreme-value taste shock of scale
$\sigma_\varepsilon$ on each option, the choice is logit and the household's
continuation value $\bar V$ is the log-sum-exp,
%
\begin{equation}
\Pr(g\mid g_-,e,a)=
\frac{\exp\{V(g\mid g_-,e,a)/\sigma_\varepsilon\}}
{\sum_{g'\in\mathcal F(g_-)}\exp\{V(g'\mid g_-,e,a)/\sigma_\varepsilon\}},
\qquad
\bar V(g_-,e,a)=\sigma_\varepsilon\log\!\!\sum_{g'\in\mathcal F(g_-)}\!\!
\exp\bigl(V(g'\mid g_-,e,a)/\sigma_\varepsilon\bigr).
\label{eq:logit}
\end{equation}
%
...
already sits inside $\coh(B,G,\cdot)$ through \eqref{eq:coh}, so the option values
...
$p_g$ & cost of living of technology $g$ & eq.~\eqref{eq:pd} \\
...
\end{equation}
%
so adopters are fully insulated from the fossil shock. Zero pass-through to the
green price is an upper bound on the channel, and we flag it as such
...
import: it is front-loaded, paid once per switcher, and scales with the switching
volume. The adopters' saved fossil energy is a resource inflow, booked as reduced
imports: it accrues gradually, over the life of each green durable,
%
\begin{equation}
\text{imports}^{\mathrm{dur}}=\psi_g\,\dsw,
\qquad
\text{energy gap}=(\PEB-\PEG)\,C^G_E.
...
- \text{imports}^{\mathrm{dur}} + \text{energy gap},
\label{eq:nx}
\end{equation}
%
and net foreign assets accumulate with a revaluation term that applies the
surprise return to the net foreign position,
%
\begin{equation}
...
terms-of-trade effect---a property inherited from ARS, not assumed anew. A standard
...
All crisis outlays are deficit-financed. Total spending is
$\text{spending}=T^{\mathrm{unt}}+T^{\mathrm{S}}+\text{Subsidy}+s_g\psi_g\dsw$, and
debt is stabilised by the distortionary labour tax,
%
\begin{equation}
B=(1+r^{\mathrm{ante}}_{-1})B_{-1}+\text{spending}-\tau^L wn,
\qquad
...
An equilibrium, given world prices and the fiscal and monetary rules, is a path of
prices $\{w,\pi,\pi^w,\pi^F,\pi^E,Q,\pE,r,r^{\mathrm{ante}}\}$, quantities
$\{y,n,c_H,c_F,c_E,C,A\}$, technology shares $\{\DG,\dsw\}$, and fiscal variables
$\{B,\tau^L\}$ such that households solve
\eqref{eq:joint}--\eqref{eq:logit}, the union and firms set wages and prices by
\eqref{eq:wpc}--\eqref{eq:pEG2}, the government follows
\eqref{eq:mp}--\eqref{eq:gbc2}, the open-economy accounts
\eqref{eq:uip}--\eqref{eq:nfa} hold, and markets clear by \eqref{eq:clearing}.
