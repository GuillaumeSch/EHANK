...
% sec:conclusion sec:monetary app:booking app:taste.
...
\end{equation}
\begin{equation}
\frac{p_G}{p_B}=\left[\frac{(1-\alpha_E)\,p_{HF/P}^{\,1-\eta_E}+\alpha_E\,(\PEG)^{1-\eta_E}}
{(1-\alpha_E)\,p_{HF/P}^{\,1-\eta_E}+\alpha_E\,(\PEB)^{1-\eta_E}}\right]^{\frac{1}{1-\eta_E}},
\qquad
\frac{p_G}{p_B}=\bigl(\PEG/\PEB\bigr)^{\alpha_E}\ \text{ for }\eta_E=1.
\label{eq:pd}
\end{equation}
%
The adoption choice responds to the relative saving $p_G/p_B<1$: the cheaper
energy an adopter enjoys, the object the switching cost buys. It is the ratio of
the two technology bundles \eqref{eq:innernest}, invariant to how the CPI is
normalised; at the calibrated steady state $p_G/p_B\approx0.992$.
...
items,\footnote{The unit of account is the CPI basket
(Section~\ref{sec:eqm}), so $p^{\num}=1$: asset returns, wages, transfers and
$\psi_g$ in \eqref{eq:coh} are already in CPI units and enter without conversion.}
%
\begin{equation}
...
%
\begin{equation}
p_d^{\,1-\eta_E}=(1-\alpha_E)\,p_{HF/P}^{\,1-\eta_E}+\alpha_E\,(P^E_d)^{1-\eta_E},
\qquad d\in\{B,G\},
\label{eq:innernest}
\end{equation}
%
...
$\sum_d \omega_d\,p_d=1$ with $\omega_d$ the consumption share of state $d$. As
green diffuses the energy leg is reweighted from brown toward green through
$\omega_G=\DG$, so $P$ is the true cost-of-living index the monetary rule reads;
no measurement gap opens between the model's price index and the cost of living.
...
\paragraph{Numeraire.}
The unit of account is the CPI basket, the AMRS convention: $p^{\num}=1$ and the
household's budget is stated directly in real terms. Because this CPI is the true
expenditure deflator, real allocations, relative prices and the market-clearing
conditions are all expressed in real terms and are independent of the unit of
account: adoption reweights the basket, and hence the price level $P$, but leaves
every real object unchanged. The CPI plays only its two economic roles, deflating
the household budget and serving as the index the monetary rule targets, with no
