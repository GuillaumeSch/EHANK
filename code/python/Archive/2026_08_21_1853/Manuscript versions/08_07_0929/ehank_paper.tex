...
\IfFileExists{output/tab_signmap_booking.tex}{\input{output/tab_signmap_booking.tex}}{\textit{[table \texttt{tab\_signmap\_booking}: run \texttt{run\_signmap.py}]}}
...
ratio rises from $1.16$ at a one-eighth shock to $2.04$ at a three-quarter
shock (Figure~\ref{fig:nonlin}, left). With the margin shut, the bare household
block is in fact mildly \emph{concave} ($y^{\mathrm{NL}}/y^{\mathrm{L}}\approx
0.92$ at the largest shock); the adoption margin is \emph{convex} and partly
