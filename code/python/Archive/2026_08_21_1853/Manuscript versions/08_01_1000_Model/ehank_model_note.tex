...
% ----------------------------------------------------------------------------
% E-HANK: complete model note (self-contained).
% Every equation below is the one implemented in the code
% (household.py / blocks.py / calibration.py / model.py). Baseline = import
% booking, core (domestic-good) numeraire, price-taking energy closure.
% ----------------------------------------------------------------------------
...
\title{\textbf{E-HANK: full model and equations}\\[2pt]
...
\begin{abstract}
\noindent This note states the complete model implemented in the code, with all
...
every aggregate block is inherited from ARS. Sections~\ref{sec:hh}--\ref{sec:agg}
give the household problem and its aggregation; Sections~\ref{sec:firms}--\ref{sec:clearing}
give firms, prices, the open economy, policy and market clearing;
Section~\ref{sec:ss} gives the steady state, shocks and calibration.
The balance-of-payments booking of the green margin is the one open modelling
choice; it is stated in Section~\ref{sec:bop} and discussed in
...
the CES demand system uses, \emph{regardless} of the unit of account.

\paragraph{Unit of account (numeraire).} The household block is
numeraire-generic. It reads three upstream objects,
\begin{equation}
p^{\num}\ \text{(price of the unit of account, relative to the CPI)},\qquad
r^{\num},\qquad atw^{\num},
\end{equation}
and returns assets denominated in units of account, converted to CPI units
downstream by $A^{CPI}=A\,p^{\num}$. Two closures are implemented:
\begin{equation}
\text{\texttt{cpi} (ARS):}\ \ p^{\num}=1,\quad r^{\num}=r,\quad atw^{\num}=atw;
\qquad
\text{\texttt{core} (baseline):}\ \ p^{\num}=p_{H/P},\quad
1+r^{\num}=(1+r)\tfrac{p_{H/P}(-1)}{p_{H/P}},\quad atw^{\num}=\tfrac{atw}{p_{H/P}} .
\end{equation}
Under \texttt{core} the unit of account is the domestic good, so the resource
constraint is independent of how the CPI weights brown and green energy. Objects
contractually written in CPI units---fiscal transfers, the ARS subsistence term,
and the switching cost $\psi_g$---are divided by $p^{\num}$ on the way into the
budget. The baseline is \texttt{core}.
...
\end{pmatrix}.
\end{equation}
On the equilibrium path $BG$ carries zero mass (no household abandons the
cheaper technology; green is lost only through breakdown, recorded as $g_-=B$),
so the effective state is three-point. In steady state a permanent green share is
sustained by a flow of switchers,
\begin{equation}
...
bound $a\ge \underline a$, $\underline a=0$ in the baseline). Cash on hand of a
household in durable state $d$ with productivity $e$ is
\begin{equation}
\coh(d,e,a)=(1+r^{\num})\,a+atw^{\num}\,e
+\frac{T^{f}+T^{\text{fisc}}+T^{\text{green}}}{p^{\num}}
-\frac{\psi_g}{p^{\num}}\,\mathrm{PAYS\_SWITCH}(d),
\label{eq:coh}
\end{equation}
with the (CPI-denominated) terms
\begin{align}
T^{f} &= -\PEB\,\bar c_E\bigl[(atw_n\,\mu_{\stst})\,s_w+(1-s_w)\bigr]
&& \text{ARS energy subsistence } (\bar c_E=0\text{ in baseline}\Rightarrow T^f=0),\\
T^{\text{fisc}} &= \epsilon^{T}+\iota\,(\pE-\bar\pE)\,\bar c^{B}_{E,i}(e,a)
&& \text{untargeted lump sum }\epsilon^T\text{ + Slutsky transfer,}\\
T^{\text{green}} &= T^{\text{green}}
&& \text{domestic green-sector rebate (}=0\text{ under import booking).}
\end{align}
Here $\iota=\texttt{insE}$ scales the Slutsky transfer and
$\bar c^{B}_{E,i}(e,a)$ is household $i$'s steady-state \emph{brown} energy
demand, collapsed over the durable axis and broadcast identically across durable
states (so a switcher keeps the same transfer; the switching margin is
undistorted). $\epsilon^{T}=\texttt{epsT}$. The switching cost enters
cash on hand only in state $GB$.
...
(margin shut). Given post-choice values $V(d)$, the choice probabilities and the
expected (pre-shock) value are the standard logit / log-sum-exp:
\begin{equation}
...
\end{align}
solved by the endogenous grid method. With $u(c)=c^{1-1/\text{eis}}/(1-1/\text{eis})$,
$u'(c)=c^{-1/\text{eis}}$. The consumption reported to welfare is net of the
(baseline-zero) GHH term; see \texttt{welfare.py}.
...
only the inner home/foreign nest, which is common to all households:
\begin{align}
c_E &= C^{B}_{E}+C^{G}_{E} \quad(+\text{ ARS subsistence, }0\text{ in baseline}),\\
...
A competitive final domestic good is produced from labour (energy in production
is off in the baseline, $\texttt{prodE\_share}=0$). With productivity $Z$ and a
constant gross markup $\mu_{\stst}$,
\begin{equation}
y = Z\,n,\qquad
...
\mathrm{div}^{\text{tot}}=\mathrm{div}+\I[\texttt{pcX\_home}=1]\,\mathrm{div}^{X},\qquad
J=\mathrm{div}^{\text{tot}}+\frac{J(+1)}{1+r^{\text{ante}}},\qquad
j=\frac{J(+1)}{1+r^{\text{ante}}}.
\end{equation}
Export pricing profit under (local- vs producer-)currency pricing is
$\mathrm{div}^{X}=\bigl(Q^{1-\varepsilon_{dcp}}p_{H/P}^{\varepsilon_{dcp}}-p_{H/P}\bigr)c_H^{*}$,
which is $0$ in the baseline ($\varepsilon_{dcp}=1$, producer-currency pricing).
...
+ \text{wedge},
\end{align}
and the nominal wage and real wage evolve as
$W=(1+\pi^{w})W(-1)$, $w=W/P$. Here $atw=atw_n/n$ is the after-tax wage per hour.
...
\paragraph{Domestic good.} With $\texttt{prodE\_share}=0$,
\begin{equation}
...
and pass-through $\rho_g=\texttt{pass\_g}$,
\begin{equation}
\PEG=\varrho\,\bar\pE\,\bigl(\PEB/\bar\PEB\bigr)^{\rho_g}.
\qquad \rho_g=0\ (\text{baseline})\Rightarrow \PEG\ \text{constant.}
\label{eq:pEG}
\end{equation}
...
\begin{equation}
c_H^{*}=\alpha^{*}\,\bigl(p_H^{*}\bigr)^{-\gamma\varepsilon_{dcp}}\,C^{*}.
\end{equation}
...
\paragraph{Importer profit centres.} Retail importers of the foreign good and of
energy earn margins over the world price, capitalised into the fund:
\begin{align}
D_F &= \bigl(p_{F/P}-Q P^{*}_{F}\bigr)c_F, & J_F&=D_F+\frac{J_F(+1)}{1+r^{\text{ante}}},\ \ j_F=\frac{J_F(+1)}{1+r^{\text{ante}}},\\
D_E &= \bigl(\pE-Q P^{E*}\bigr)(c_E+\mathrm{prodE}), & J_E&=D_E+\frac{J_E(+1)}{1+r^{\text{ante}}},\ \ j_E=\frac{J_E(+1)}{1+r^{\text{ante}}}.
\end{align}
(Under the domestic booking the energy importer margin is levied on brown energy
$C^{B}_{E}$ only.)
...
its rents are capitalised, and the home economy owns a share $\zeta^{E}$:
\begin{align}
E_{\text{stock}} &= \frac{P^{E*}(+1)/(1+r^{*})-P^{E*}}{\Gamma_{\text{arb}}},\qquad
E^{\text{supply}} = E^{\text{shock}}+\bigl(E_{\text{stock}}(-1)-E_{\text{stock}}\bigr),\\
D_{E^{*}} &= Q\,P^{E*}\,E^{\text{supply}},\qquad
J_{E^{*}}=D_{E^{*}}+\frac{J_{E^{*}}(+1)}{1+r^{\text{ante}}},\qquad
j_{E^{*}}=\frac{J_{E^{*}}(+1)}{1+r^{\text{ante}}}.
\end{align}

\paragraph{Fund return (revaluation).} The realised (ex post) return on the
fund's claims defines $r$:
\begin{equation}
1+r=\frac{J+\zeta_F J_F+\zeta_E J_E+\zeta^{E}J_{E^{*}}}
{j(-1)+\zeta_F j_F(-1)+\zeta_E j_E(-1)+\zeta^{E}j_{E^{*}}(-1)},
\qquad (\zeta_F=\zeta_E=0\ \text{baseline}),
\end{equation}
and analogously the domestic-fund return $r^{\text{dom}}$ and value $A^{\text{dom}}$
(used only in the current-account revaluation term).

\paragraph{Balance of payments (import booking, baseline).}\label{sec:bop}
Let the two durable-margin terms be
\begin{equation}
...
\begin{align}
\text{exports}&=Q\bigl(p_H^{*}c_H^{*}+P^{E*}\zeta^{E}E^{\text{supply}}\bigr),\\
\text{imports}&=Q\bigl(P^{*}_{F}c_F+P^{E*}(c_E+\mathrm{prodE})\bigr)+\text{imports}_{\text{dur}}-\text{energy\_gap},\\
nx&=Q\bigl(p_H^{*}c_H^{*}-P^{*}_{F}c_F-P^{E*}(c_E+\mathrm{prodE}-\zeta^{E}E^{\text{supply}})\bigr)-\text{imports}_{\text{dur}}+\text{energy\_gap}.
\end{align}
...
The nominal rate follows an inertial rule; the ex-ante real rate is Fisher:
\begin{align}
1+i &= \rho_i(1+i(-1))+(1-\rho_i)(1+\bar r^{*})\,
(1+\phi_\pi\pi)(1+\phi_{\pi w}\pi^{w})(1+\phi_{\pi e}\pi(+1))+\text{ishock},\\
r^{\text{ante}} &= \frac{1+i}{1+\pi(+1)}-1.
\end{align}
The baseline is the ARS \emph{constant real rate} rule
$(\rho_i,\phi_\pi,\phi_{\pi w},\phi_{\pi e})=(0,0,0,1)\Rightarrow r^{\text{ante}}=\bar r^{*}$;
a standard Taylor rule sets $\phi_\pi=1.5,\phi_{\pi e}=0$. \texttt{ishock} is a
monetary shock.

\subsection{Fiscal policy}\label{sec:fiscal}

The government runs the two energy-crisis instruments and a lump sum, all
debt financed, with debt stabilised through the distortionary labour tax.
Define the fiscal base of the subsidy (booking-dependent):
\begin{equation}
\text{base}_S=\begin{cases} c_E & \text{import booking}\\ C^{B}_{E} & \text{domestic booking.}\end{cases}
\end{equation}
Then
\begin{align}
\text{Subsidy} &= \tau^{E}\,(\pE-\bar\pE)\,\text{base}_S
...
\text{taxation}=\tau^{Y}\,\text{btw}_n=\tau^{Y}\,w\,n. &&
\end{align}
The government budget constraint and the debt-stabilising tax rule are
\begin{align}
B &= (1+r^{\text{ante}}(-1))\,B(-1)+\text{spending}-\text{taxation},
\label{eq:gbc}\\
\tau^{Y} &= (1-\texttt{bb})\,\psi_B\bigl(B(-1)-\bar B\bigr)+\texttt{bb}\,(B-\bar B),
\label{eq:taurule}
\end{align}
with feedback coefficient $\psi_B=\texttt{psiB}$. \textbf{In the baseline
$\bar B=0$ (no steady-state government debt) and $\texttt{bb}=0$, so
\eqref{eq:taurule} reduces to $\tau^{Y}=\psi_B(B(-1)-\bar B)$}: crisis outlays
are deficit-financed and repaid by raising the labour tax in proportion to the
debt's deviation from zero. Government debt $B$ is a genuine transition unknown
(it moves in the dynamics) and enters household wealth through asset market
clearing, eq.~\eqref{eq:assets}.
...
\text{assets (Walras):}&\quad A^{CPI}-\mathrm{nfa}-j-j_F-j_E-B-\zeta^{E}j_{E^{*}}=0, \label{eq:assets}\\
\text{energy:}&\quad
\begin{cases}
P^{E*}-P^{E*,\text{shock}}=0 & \varepsilon^{E}=\infty\ \text{(price taker; price shock)},\\
(c_E+\mathrm{prodE})-E^{\text{supply}}=0 & \varepsilon^{E}<\infty\ \text{(fixed quantity; supply shock).}
\end{cases}\label{eq:energy}
\end{align}
Under the domestic booking, only brown energy clears against world supply in
\eqref{eq:energy}: $(C^{B}_{E}+\mathrm{prodE})-E^{\text{supply}}=0$.
...
$\{\text{goods}, \text{$\pi$ residual}, r\text{ residual}, \tau^{Y}\text{ residual},
\text{energy}, w\text{ residual}\}$; under the domestic booking $T^{\text{green}}$
is an added unknown with residual~\eqref{eq:tgreen}.

% ============================================================================
\section{Steady state, shocks, calibration}\label{sec:ss}
% ============================================================================

\subsection{Openness and endowment normalisations}

The openness parameters and the ARS home-endowment normalisation
($z\equiv\zeta^{E}$, $a_E\equiv\alpha_E$) are
\begin{align}
\alpha_F&=\frac{\alpha-\alpha_E}{1-\alpha_E}, &
\gamma&=\frac{\chi_{\text{tgt}}-(1-\alpha)(1-\alpha_F)\eta_E}{(1-\alpha)\alpha_F+\alpha},\quad \eta=\gamma,\ \ \chi_{\text{tgt}}=0.3,\\
y&=1-z\,a_E, & \mu_{\stst}&\leftarrow\mu_{\stst}(1-z\,a_E),\quad
\alpha^{*}=\alpha-a_E z,\quad Z=\mu_{\stst},\quad E^{\text{shock}}=a_E .
\end{align}
So the gross markup entering firm profits is $\mu_{\stst}(1-\zeta^{E}\alpha_E)\approx 1.016$;
$1.03$ is the pre-normalisation primitive.

\subsection{Steady-state unknowns/targets}

The steady state solves for $\{\varphi,\ \beta_{\max},\ y,\ \psi_g\}$ against
$\{\pi^{w}\text{ res},\ \mathrm{nfa}\text{ res},\ \text{goods},\ \DG-0.05\}$, i.e.\
$\psi_g$ is calibrated to the level target $\DG_{\stst}=0.05$ and $\beta_{\max}$ to
close net foreign assets. Under the domestic booking $T^{\text{green}}$ is an
added unknown closed by the zero-profit green-sector residual
\begin{equation}
T^{\text{green}}=\PEG\,C^{G}_{E}+\psi_g\,\dsw . \label{eq:tgreen}
\end{equation}
In the ``no-adoption'' counterfactual ($\texttt{green\_block}$ large) $\DG\equiv 0$
for every $\psi_g$, so $\psi_g$ is not identifiable and is carried over, fixed,
from the matching adoption steady state.
...
$\zeta^{E}$ home energy-rent share & 0.33 & ARS\\
\midrule
...
% ============================================================================
\section{Remark on the balance-of-payments booking}\label{sec:remarks}
% ============================================================================

The one open modelling choice is how the green margin enters the balance of
payments. The code implements two internally consistent bookings:
\begin{enumerate}[label=(\roman*),leftmargin=2em]
\item \textbf{Import (baseline above).} The switching cost $\psi_g\dsw$ is
booked as an import and the adopter's energy saving $(\PEB-\PEG)C^{G}_{E}$ as an
import reduction; all energy is imported at the world brown price
(Section~\ref{sec:bop}).
\item \textbf{Domestic.} Green energy $\PEG C^{G}_{E}$ and installation
$\psi_g\dsw$ are domestic value added rebated lump-sum ($T^{\text{green}}$,
eq.~\eqref{eq:tgreen}); only brown energy is imported and clears against world
supply. It is an \emph{upper bound} on domestic value added (zero marginal cost).
\end{enumerate}
The choice has a first-order effect on the adoption channel's \emph{output} sign.
Per the current plan we intend to fix a \emph{single} baseline: adoption
purchases (the switching cost) expensed as the \emph{core consumption good} (the
home/foreign composite, split by $\alpha$), and \emph{green energy imported} on
the same footing as brown. This is a third, intermediate convention---between
(i) and (ii)---and is the most standard treatment (a durable installation is an
ordinary consumption-good purchase). It removes the auxiliary channel that
distinguishes brown from green energy at the balance-of-payments level, at the
cost of re-specifying \texttt{hh\_outputs\_dur} (route $\psi_g\dsw$ through
$C_{HF}$), the current account, and the energy importer margin.

% ============================================================================
\begin{thebibliography}{9}
\bibitem{ARS2024} Auclert, A., Monnery, H., Rognlie, M., Straub, L. (2024).
\emph{Managing an Energy Shock}. Working paper.
\bibitem{Bayer2026} Bayer, C., et al. (forthcoming). \emph{Redistribution Within
and Across Borders}. Journal of Political Economy Macroeconomics.
\bibitem{Langot2026} Langot, F., et al. (forthcoming). \emph{The Macroeconomic
and Redistributive Effects of the French Tariff Shield}. Journal of Political
