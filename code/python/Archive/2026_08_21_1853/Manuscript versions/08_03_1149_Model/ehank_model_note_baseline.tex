...
every aggregate block is inherited from ARS. The note documents the baseline
...
D_E &= \bigl(\pE-Q P^{E*}\bigr)c_E, & J_E&=D_E+\frac{J_E(+1)}{1+r^{\text{ante}}},\ \ j_E=\frac{J_E(+1)}{1+r^{\text{ante}}}.
\end{align}
...
\begin{equation}
1+r=\frac{J+\zeta^{E}J_{E^{*}}}{j(-1)+\zeta^{E}j_{E^{*}}(-1)}.
\end{equation}
A domestic-fund counterpart $r^{\text{dom}},A^{\text{dom}}$ (value of the
domestically held claims, $A^{\text{dom}}=j+j_F+j_E+\zeta^{E}j_{E^{*}}$) is also
computed and enters only the current-account revaluation term below.
...
\text{imports}&=Q\bigl(P^{*}_{F}c_F+P^{E*}c_E\bigr)+\text{imports}_{\text{dur}}-\text{energy\_gap},\\
nx&=Q\bigl(p_H^{*}c_H^{*}-P^{*}_{F}c_F-P^{E*}(c_E-\zeta^{E}E^{\text{supply}})\bigr)-\text{imports}_{\text{dur}}+\text{energy\_gap}.
\end{align}
