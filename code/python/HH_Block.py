...
#%% Import the DD calibration
with open('calibration_ss_DD.json', 'r') as f:
    ss_DD = json.load(f)
    
cali['baseline_DD'] = deepcopy(cali['baseline'])
for key in ss_DD.keys():
    if key in cali['baseline_DD']:
        cali['baseline_DD'][key] = ss_DD[key]
        
        
#evaluate_param_changes('tau_b', [0.01,0.10,0.2, 0.5], ha, cali['baseline_DD'],
#                       ss_vars = ['tau_b', 'D_N', 'D_B', 'D_G', 'asset_mkt', 'C'])

evaluate_param_changes('B', [0.1], ha, cali['baseline_DD'],
                       ss_vars = ['B', 'D_N', 'D_B', 'D_G', 'asset_mkt', 'C', 'A','Tax'])
        
make_consu_bundle_price(p_core, n_b, p_e_b, n_g, p_e_g, tau_b, tau_g, xi, nu)
        

#ss_DD = ha.steady_state(cali['baseline_DD'])
#print("Nouvelle calibration chargée et appliquée.")
